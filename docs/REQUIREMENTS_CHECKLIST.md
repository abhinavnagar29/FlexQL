...
| Caching | Client-side SELECT cache defaults ON (unset) and can be disabled via `FLEXQL_QUERY_CACHE=0`; server-side SELECT response cache defaults ON (unset) and can be disabled via `FLEXQL_SERVER_SELECT_CACHE=0` | Caches are invalidated on write; disable caches for strict “always hit storage” evaluation by setting both env vars to `0`. |
